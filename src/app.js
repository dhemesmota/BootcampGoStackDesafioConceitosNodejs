...
app.put("/repositories/:id", validateProjectId, (request, response) => {
  ...
  const repositoryData = repositories[repositoryIndex];

  const repository = { id, title, url, techs, likes: repositoryData.likes };
...
app.post("/repositories/:id/like", validateProjectId, (request, response) => {
  ...
  const repository = { ...repositoryData, likes: repositoryData.likes + 1 };
